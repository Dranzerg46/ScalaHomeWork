...
object subtyping:
  ...
  def observeCounterList(f: List[Counter] => Unit): List[Int] =
    ...
    val z: C = new C
    f(x::y::z::Nil)
    val xResult: Int = x.get()
    val yResult: Int = y.get()
    val zResult: Int = z.get()
    xResult::yResult::zResult::Nil
    //throw UnsupportedOperationException()
  ...
  def observeCounterArray(f: Array[Counter] => Unit): Array[Int] =
    ...
    val y : C = new C
    val z : C = new C
    val first : List[Counter] = x::y::z::Nil
    f(first.toArray)
    val xResult: Int = x.get()
    val yResult: Int = y.get()
    val zResult: Int = z.get()
    val firstResult : List[Int] = xResult::yResult::zResult::Nil
    firstResult.toArray
    //throw UnsupportedOperationException()
