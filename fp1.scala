...
object fp1:
  ...
  //if (p == null) {throw new MathError(p)}
    val x : Int = p(0)
    val y : String = p(1)
    (p(1),p(0))
    //throw UnsupportedOperationException()
  ...
  def maxListHelp(xs: List[Int]): Int =
      xs match
        case Nil => 0
        case y :: ys => y max maxListHelp(ys)


  def maxList(xs: List[Int]): Int =
    if xs == Nil then throw NoSuchElementException()
    else  
      maxListHelp(xs)
  ...
  def maxTailAux(xs: List[Int], accumulator: Int): Int =
      xs match
        case Nil => accumulator
        case y :: ys => maxTailAux(ys, accumulator max y)
  //  throw UnsupportedOperationException()
  ...
  def otpuHelp(start: Int, counter: Int, xs: List[Int]): List[Int] =
    if start < counter then xs
    else
      var ys = counter :: xs
      otpuHelp(start, counter+1, ys)
    
      

  def otpu(start: Int, end: Int): List[Int] =
    if end > start then Nil
    else
      var xs = List (end)
      otpuHelp(start, end+1,xs)
