...
object fp3:
  ...
  def stringLengths(xs: List[String]): List[(String, Int)] =
    xs match
      case Nil => List()
      case y::ys => (y, y.length())::stringLengths(ys)
  ...
  def delete1[X](x: X, ys: List[X]): List[X] =
    ys match
      case Nil => Nil
      case z::zs => if (z == x) then delete1(x,zs) else z::delete1(x,zs)
  ...
  def removeDupes1[X](xs: List[X]): List[X] = 
    // TODO: Provide definition here.
    def dupeHelp [X] (temp:X, ys:List[X]) : List[X] =
      ys match
        case Nil => if temp != Nil then temp::Nil else Nil
        case w::ws => if w != temp then temp::dupeHelp(w,ws) else dupeHelp(w,ws)
      
    xs match
     case Nil => Nil
     case z::Nil => List(z)
     case z::zs => dupeHelp(z,zs)
  ...
  def removeDupes2[X](xs: List[X]): List[(Int, X)] =
    // TODO: Provide definition here.
    // TODO: Provide definition here.
    //https://www.geeksforgeeks.org/scala-nested-functions/#:~:text=In%20Scala%2C%20we%20can%20define,called%20nested%20or%20local%20functions.
    def dupeHelp2[X] (temp:X, ys:List[X], count:Int) : List[(Int, X)] =
      ys match
        case Nil => if temp != Nil then (count,temp)::Nil else Nil
        case w::ws => if w != temp then (count,temp)::dupeHelp2(w,ws,1) else dupeHelp2(w,ws,count+1)
      
    xs match
     case Nil => Nil
     case z::Nil => List((1,z))
     case z::zs => dupeHelp2(z,zs,1)
   // throw UnsupportedOperationException()
  ...
  def splitAt[X](n: Int, xs: List[X]): (List[X], List[X]) =
    // TODO: Provide definition here.
   // https://stackoverflow.com/questions/5392922/how-do-i-match-multiple-arguments
   // i got how to to multiple arguments from here
  (n, xs) match
    case (_, Nil) => (Nil,Nil)
    case(z,y::ys) => val (x1, x2) = splitAt(n-1,ys) 
     if z > 0 then (y::x1,x2) else (x1,y::x2)
